...
class TestLector(TestCase):
    ...
    def test_lee_archivo_json(self):
        ...
        lista_resultado = df.to_dict(orient='records')  # Modifico el dataframe a lista de diccionarios
        self.assertListEqual(expected_list, lista_resultado) # Porque el test espera una lista de diccionarios
    ...
    def test_lee_archivo_txt(self):
        ...
        expected_list = [
            ...
            {'id': 'VY1603', 'fecha_llegada': '08/05/2022T08:45', 'retraso': '-', 'tipo_vuelo': 'INTERNAT',
             'destino': 'Nueva York'}, # modifico de NuevaYork a Nueva York
            {'id': 'VY3302', 'fecha_llegada': '08/05/2022T12:30', 'retraso': '00:15', 'tipo_vuelo': 'NAT',
             'destino': 'Bruselas'}]
        lector = LectorTXT(path)
        df = lector.lee_archivo()
        lista_resultado = df.to_dict(orient='records')  #  Lo mismo q en json, paso dataframe a lista de diccionarios
        self.assertListEqual(expected_list, lista_resultado) # Porque el test espera una lista de diccionarios
